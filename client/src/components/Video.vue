  ...
  methods: {
    ...
    async getAccessToken() {
      return await axios.get(
        `http://localhost:4000/token?identity=${this.username}`
      );
    },
    ...
    createChat(room_name) {
      ...
      this.getAccessToken().then(data => {
        ...
        /**********************************************************************************
         *  before a user enters a new room, disconnect the user from they joined already *
         **********************************************************************************/
        this.leaveRoomIfJoined();

        /******removing any remote track when joining a new room**********/

        document.getElementById("remoteTrack").innerHTML = "";

        connect(token, connectOptions).then(function(room) {
